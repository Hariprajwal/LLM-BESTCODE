...
// Use a different port to avoid conflict with main server
const PORT = process.env.LLM_PORT || 3001;
const MODEL_NAME = "codellama";

// Root endpoint
app.get("/", (req, res) => {
  res.json({ message: "Smart Code Hub LLM WebSocket server is running" });
});
...
wss.on("connection", (socket) => {
  console.log("LLM User connected");
  ...
  socket.on("message", async (userInput) => {
    try {
      ...
      const response = await fetch('http://10.255.255.254:11434/api/generate', {
        method: 'POST',
        headers: {
  ...
  socket.on("close", () => {
    console.log("LLM User disconnected");
  });
});
